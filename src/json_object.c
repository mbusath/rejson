...
void push_callback(jsonsl_t jsn, jsonsl_action_t action,
...
void pop_callback(jsonsl_t jsn, jsonsl_action_t action,
                  struct jsonsl_state_st *state, const jsonsl_char_t *at) {
  json_object_context_t *joctx = (json_object_context_t *)jsn->data;
  ...
  if (JSONSL_T_STRING == state->type) {
    n = NewStringNode(jsn->base + state->pos_begin + 1,
                      state->pos_cur - state->pos_begin - 2);
    ...
    n = NewKeyValNode(jsn->base + state->pos_begin + 1,
                      state->pos_cur - state->pos_begin - 2, NULL);
    if (!n) goto alloc_error;
    joctx->nodes[joctx->len++] = n;
  }
  ...
  if (state->type != JSONSL_T_HKEY) {
    if (joctx->len > 1) {
      if (joctx->nodes[joctx->len - 2]->type == N_KEYVAL) {
        if (OBJ_ERR ==
            Node_DictSet(joctx->nodes[joctx->len - 3],
        ...
        if (OBJ_ERR == Node_ArrayAppend(joctx->nodes[joctx->len - 2],
                                        joctx->nodes[joctx->len - 1]))
          goto alloc_error;
        joctx->len -= 1;
      }
...
int error_callback(jsonsl_t jsn, jsonsl_error_t err,
...
Node *CreateNodeFromJSON(const char *buf, size_t len, char *err) {
  ...
  if (!(joctx->nodes = calloc(JSONSL_MAX_LEVELS, sizeof(Node *))))
    goto alloc_error;
