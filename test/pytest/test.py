...
class JSONTestCase(ModuleTestCase(module_path='../../lib/rejson.so', redis_path='../../../redis/src/redis-server')):
    ...
    def testMgetCommand(self):
        with self.redis() as r:
            for d in range(0, 5):
                ...
                r.delete(key)
                self.assertOk(r.execute_command(
                    'JSON.SET', key, '.', json.dumps(docs['basic'])))

            raw = r.execute_command(
                'JSON.MGET', '.', *['doc:{}'.format(d) for d in range(0, 5)])
            ...
            r.delete('test')
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.', '{"bool":false}'))
            raw = r.execute_command(
                'JSON.MGET', '.bool', 'test', 'doc:0', 'foo')
    ...
    def testArrayCRUD(self):
        with self.redis() as r:
            ...
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.', '{ "arr": [] }'))
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.arr[-inf]', 0))
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.arr[+inf]', 1))
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.arr[0]', 0.1))
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.arr[-1]', 0.9))
            ...
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[0]', 0), 2)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[-inf]', -1), 3)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[+inf]', 2), 4)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[-4]', -2), 5)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[6]', 4), 6)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[-1]', 3), 7)
            data = json.loads(r.execute_command('JSON.GET', 'test', '.arr'))
            ...
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.', '{ "arr": [] }'))
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[0]', 2), 1)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[-1]', 0, 1), 3)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[-inf]', -2, -1), 5)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[+inf]', 3, 6), 7)
            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[6]', 4, 5), 9)
            data = json.loads(r.execute_command('JSON.GET', 'test', '.arr'))
    ...
    def testIndexCommand(self):
        with self.redis() as r:
            ...
            self.assertOk(r.execute_command('JSON.SET', 'test',
                                            '.', '{ "arr": [0, 1, 2, 3, 2, 1, 0] }'))
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 0), 0)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 3), 3)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 4), -1)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 0, 1), 6)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 0, -1), 6)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 0, 6), 6)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 0, 4, -1), 6)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 0, 5, -2), -1)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 2, -2, 6), -1)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', '"foo"'), -1)

            self.assertEqual(r.execute_command(
                'JSON.INSERT', 'test', '.arr[4]', '[4]'), 8)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 3), 3)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', 2, 3), 5)
            self.assertEqual(r.execute_command(
                'JSON.INDEX', 'test', '.arr', '[4]'), -1)
    ...
    def testTypeCommand(self):
        with self.redis() as r:
            r.delete('test')
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.', json.dumps(docs['types'])))
    ...
    def testLenCommand(self):
        with self.redis() as r:
            ...
            self.assertEqual(r.execute_command(
                'JSON.LEN', 'foo', '.bar'), None)

            # test elements with valid lengths
            self.assertOk(r.execute_command(
                'JSON.SET', 'test', '.', json.dumps(docs['basic'])))
            self.assertEqual(r.execute_command(
                'JSON.LEN', 'test', '.string'), 12)
            ...
            self.assertEqual(r.execute_command(
                'JSON.LEN', 'test', '.bool'), -1)
            self.assertEqual(r.execute_command(
                'JSON.LEN', 'test', '.none'), -1)
